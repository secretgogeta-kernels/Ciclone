...
int main(int argc, char* argv[]) {
             ...
             g_threadPrivateKeys)
    {
        ...
        Point startPoint = secp.ComputePublicKey(&privateKey);
        ...
        while (true) {
                            ...
                            if (!matchFound && std::memcmp(localHashResults[j], targetHash160.data(), 20) == 0) {
                                #pragma omp critical
                                {
                                    if (!matchFound) {
                                        ...
                                        matchingPrivateKey.Set(&privateKey);
            ...
            {
                Int step;
                step.SetInt32(fullBatchSize - 2);
                privateKey.Add(&step);
                startPoint = secp.AddDirect(startPoint, big512G);
            }
